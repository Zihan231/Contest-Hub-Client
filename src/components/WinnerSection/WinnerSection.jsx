        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          {winners.map((winner, index) => (
            <div 
                ...
                ${index === 1 ? 'md:-mt-8 md:mb-8 border-primary/50 shadow-primary/20' : ''}`} // Elevate middle card
            >
              <div className="card-body items-center text-center">
                
                {/* Image Ring */}
                <div className={`w-24 h-24 rounded-full p-1 border-4 mb-4
                ...
                <p className="text-sm text-base-content/60 font-medium mb-4">{winner.contest}</p>

                {/* Prize Tag */}
                <div className="badge badge-lg py-4 px-6 gap-2 bg-base-200 border-none text-base-content shadow-inner">
                    <FaTrophy className={index === 0 ? "text-yellow-500" : index === 1 ? "text-gray-400" : "text-orange-500"} />
                    <span className="font-bold">{winner.prize} Won</span>
                </div>
              </div>
            </div>
          ))}
