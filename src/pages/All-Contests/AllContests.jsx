...
import { FaUsers, FaArrowRight, FaSearch, FaPenNib, FaCode, FaBusinessTime, FaLayerGroup, FaPaintBrush, FaTags } from "react-icons/fa";
...
const AllContests = () => {
    ...
    const filteredContests = allContests.filter((contest) => {
        ...
        const typeMatch =
            active === "all" || type === active || tagList.includes(active);
                        ...
                        <div className="flex justify-start min-w-max gap-1">
                            {tabs.map((tab) => (
                                <button
                                    ...
                                    className={`group relative flex items-center gap-2 px-4 py-3 text-sm font-bold transition-all duration-300 ${activeTab === tab.id ? "text-primary" : "text-base-content/60 hover:text-base-content"}`}
                                >
                                    <span className={`text-lg ${activeTab === tab.id ? 'scale-110' : 'group-hover:scale-110'} transition-transform`}>{tab.icon}</span>
                                    <span className="tracking-wide uppercase text-xs">{tab.label}</span>
                                    <span className={`absolute bottom-0 left-0 h-0.5 w-full bg-primary transform transition-transform duration-300 origin-center z-10 ${activeTab === tab.id ? "scale-x-100" : "scale-x-0 group-hover:scale-x-50"}`}></span>
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                {paginatedContests.length > 0 ? (
                    <>
                            ...
                            {paginatedContests.map((contest, index) => (
                                ...
                                >
                                    <figure className="relative h-48 overflow-hidden bg-base-200">
                                        <img
                                            src={contest.image || "https://placehold.co/600x400?text=Contest"} // Fallback image
                                            alt={contest.contestName}
                                            className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-500"
                                            onError={(e) => e.target.src = "https://placehold.co/600x400?text=No+Image"} // Handle broken links
                                        />
                        ...
                        <div className="flex justify-center items-center gap-2">
                            ...
                            {/* Mobile simplified pagination display */}
